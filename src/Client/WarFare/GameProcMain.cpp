...
{
	m_fMsgSendTimeMove      = 0.0f;
	m_fMsgSendTimeRot       = 0.0f;
	m_fPlayerYawSended      = 0.0f;
	m_vPlayerPosSended      = {};
	m_vMouseLBClickedPos    = {};
	m_vMouseSkillPos        = {};
...
void CGameProcMain::InitPlayerPosition(const __Vector3& vPos)              // 플레이어 위치 초기화.. 일으켜 세우고, 기본동작을 취하게 한다.
{
	...
	float fYObject      = ACT_WORLD->GetHeightNearstPosWithShape(vPos);    // 오브젝트에서 가장 가까운 높이값 얻기..
	if (!s_pWorldMgr->IsIndoor())
	{
		if (std::abs(vPos.y - fYObject) < std::abs(vPos.y - fYTerrain))
...
//	ProcessPlayerInclination();							// 경사 처리..(가만히 있어도 경사가 급하면 미끄러짐..).
	...
	if (m_bIsExitCanceled && m_fExitTimer != -1.0f)
	{
	...
	if (m_eExitType != EXIT_TYPE_NONE)
	{
		...
		if (m_iExitTimeRemaining > secondsRemaining)
		{
			...
			{
				std::string szMsg = fmt::format_text_resource(IDS_EXITING_GAME_IN_X_SECONDS, m_iExitTimeRemaining);
			...
			if (secondsRemaining <= 0)
			{
				...
				else if (m_eExitType == EXIT_TYPE_CHR_SELECT && m_pUIExitMenu != nullptr)
				{
					m_eExitType          = EXIT_TYPE_NONE;
					m_iExitTimeRemaining = EXIT_TIME_AFTER_BATTLE;
					m_bIsExitCanceled    = FALSE;
	...
	if (m_fMBRotateTime >= 0.0f && !s_pUIMgr->m_bDoneSomething)
	{
		...
		if (fRadian != 0.0f && s_pPlayer->IsAlive())
		{
...
void CGameProcMain::RenderTarget()
{
	...
	if (nullptr == pTarget && nullptr == s_pPlayer->m_pObjectTarget)
		return;
...
bool CGameProcMain::ProcessPacket(Packet& pkt)
{
	...
	switch (iCmd)
	{
		...
		case WIZ_WARP:
		{
			...
			float fY       = ACT_WORLD->GetHeightWithTerrain(fX, fZ);
		...
		case WIZ_FRIEND_PROCESS:
			if (m_pUIVar->m_pPageFriends)
				m_pUIVar->m_pPageFriends->MsgRecv_MemberInfo(pkt);
			return true;
		...
		case WIZ_PARTY_BBS:
			if (m_pUIPartyBBS)
				m_pUIPartyBBS->MsgRecv_RefreshData(pkt);
			return true;
		case WIZ_MARKET_BBS:
			if (m_pUITradeBBS)
				m_pUITradeBBS->MsgRecv_TradeBBS(pkt);
			return true;
		case WIZ_SELECT_MSG:
			if (m_pUIQuestMenu)
				m_pUIQuestMenu->Open(pkt);
			return true;
		case WIZ_NPC_SAY:
			if (m_pUIQuestTalk)
				m_pUIQuestTalk->Open(pkt);
			return true;
			//		case N3_CLAN:
...
void CGameProcMain::ProcessLocalInput(uint32_t dwMouseFlags)
{
	...
	if (s_pEng->ViewPoint() == VP_THIRD_PERSON)
	{
		...
		if (s_pLocalInput->IsKeyDown(DIK_HOME))
			fPitch = DegreesToRadians(45.0f);  // home 키가 눌리면..
		else if (s_pLocalInput->IsKeyDown(DIK_END))
			fPitch = DegreesToRadians(-45.0f); // End 키가 눌리면..
		if (fPitch)
	...
	if (!IsUIKeyOperated() && nullptr == CN3UIBase::GetFocusedEdit()) // 채팅모드가 아닐때
	{
		...
		else if (s_pLocalInput->IsKeyDown(KM_MOVE_BACKWARD) || s_pLocalInput->IsKeyDown(DIK_DOWN))
		{
			...
			CommandMove(MD_BACKWARD, bStart); // 뒤로 이동..
		}
		...
		if (s_pLocalInput->IsKeyPress(KM_TOGGLE_INVENTORY))
			CommandToggleUIInventory();

		if (s_pLocalInput->IsKeyPress(KM_TOGGLE_STATE))
			CommandToggleUIState();

		if (s_pLocalInput->IsKeyPress(KM_TOGGLE_SKILL))
			CommandToggleUISkillTree();

		if (s_pLocalInput->IsKeyPress(KM_TOGGLE_CMDLIST))
			CommandToggleCmdList();

		if (s_pLocalInput->IsKeyPress(KM_TOGGLE_SITDOWN))
			CommandSitDown(true, !s_pPlayer->m_bSitDown);
...
void CGameProcMain::MsgSend_Continous() // 특정 조건(?)하에서 서버에게 정기적으로 메시지를 보냄..
{
	...
	if (fTime >= m_fMsgSendTimeMove + PACKET_INTERVAL_MOVE) // 1초가 지났으면..
	{
		...
		if (m_vPlayerPosSended != vPos)
			MsgSend_Move(true, true);                                                        // 조금이라도 움직였으면 움직임 패킷 보냄..
	}

	if (!s_pPlayer->m_bMoveContinous && fTime >= m_fMsgSendTimeRot + PACKET_INTERVAL_ROTATE) // 플레이어가 정지해 있고.. 2초가 지났으면..
	{
...
{
	if (nullptr == pCorpse || pCorpse->m_iDroppedItemID <= 0)
		return false;
...
void CGameProcMain::MsgSend_PartyOrForcePermit(bool bYesNo)
{
	uint8_t byBuff[4];
	int iOffset = 0;
...
bool CGameProcMain::MsgSend_PartyOrForceCreate(const std::string& szID)
{
	if (szID.empty() || szID.size() > MAX_ID_SIZE)
		return false;

	bool bIAmLeader = false, bIAmMember = false;
	int iMemberIndex     = -1;
...
void CGameProcMain::MsgSend_PartyOrForceLeave()
{
	...
	bool bIAmLeader = false, bIAmMember = false;
	int iMemberIndex = -1;
	PartyOrForceConditionGet(bIAmLeader, bIAmMember, iMemberIndex, pTarget);
	...
	for (int i = 0; i < ITEM_SLOT_COUNT; i++) // 슬롯 갯수마큼..
	{
		...
		switch (eSlot)
		{
			...
			case ITEM_SLOT_HAND_LEFT:                                  // 왼손
			{
				...
				e_PlugPosition ePlugPos = PLUG_POS_UNKNOWN;
...
bool CGameProcMain::MsgRecv_Chat(Packet& pkt)
{
	...
	switch (eCM)
	{
		...
		case N3_CHAT_PRIVATE:
			crChat = D3DCOLOR_ARGB(255, 128, 255, 255); /*D3DCOLOR_ARGB(255,192,192,0);*/
			break;
	...
	{
		if (eNation != s_pPlayer->Nation() && !s_pPlayer->m_InfoExt.bCanTalkToOtherNation)
		{
...
bool CGameProcMain::MsgRecv_UserMove(Packet& pkt)
{
	...
	//	if(0.0f != fSpeed) TRACE("Move - {:4.1f} ( {} : Mode {} ) {:.2f}", fSpeed, iID, byMoveFlag, CN3Base::TimeGet());
	//	else TRACE("Stop - {:4.1f} ( {} : Mode {} )  {:.2f}", fSpeed, iID, byMoveFlag, CN3Base::TimeGet());
...
*/
	...
	//TRACE("Weather updated [type={} amount={}]", iWeather, iPercent);

	ACT_WORLD->SetWeatherWithSky((CN3SkyMng::eSKY_WEATHER) iWeather, iPercent); // 하늘 변화하기
	float fDelta = 1.0f;
	if (0x1 == iWeather)                                                        // 맑은 날씨. 퍼센트는 안개...
		fDelta = 0.5f + (1.0f - fPercent) * 0.5f;
	else if (0x02 == iWeather                                                   // 비옴 .
			 || 0x03 == iWeather)                                               // 눈옴..
...
bool CGameProcMain::MsgRecv_UserInOut(Packet& pkt)
{
	...
	if (iType == 0x01
		// 유저 들어올때 (warp)
		|| iType == 0x04)
		MsgRecv_UserIn(pkt, false);
	// User 나갈때.
	else if (iType == 0x02)
		MsgRecv_UserOut(pkt);
	// 유저 들어올때 (죽었다 살아나거나, game start하는 유저들.)
	else if (iType == 0x03)
		MsgRecv_UserIn(pkt, true);
...
bool CGameProcMain::MsgRecv_UserIn(Packet& pkt, bool bWithFX)
{
	...
	if (pUPC != nullptr)
	{
		CLogWriter::Write("User In - Duplicated ID ({}, {}) Pos({:.2f},{:.2f},{:.2f})", iID, szName, fXPos, fYPos, fZPos);
		//TRACE("User In - Duplicated ID ({}, {}) Pos({:.2f},{:.2f},{:.2f})", iID, szName, fXPos, fYPos, fZPos);
	...
	D3DCOLOR crID = (eNation != s_pPlayer->m_InfoBase.eNation) ? D3DCOLOR_XRGB(255, 96, 96)
...
bool CGameProcMain::MsgRecv_NPCIn(Packet& pkt)
{
	...
	{
		CLogWriter::Write("NPC In - Duplicated ID ({}, {}) Pos({:.2f},{:.2f},{:.2f})", iID, szName, fXPos, fYPos, fZPos);
		//TRACE("NPC In - Duplicated ID ({}, {}) Pos({:.2f},{:.2f},{:.2f})", iID, szName, fXPos, fYPos, fZPos);
	...
	if (0 == dwType)
	{
		...
		if (nullptr == pLooks)                    // 캐릭터 기본 모습 테이블이 없으면...
		{
			...
			std::string szMsg = fmt::format("Normal NPC In : [Name({}), ResourceID({})]", szName, iIDResrc);
	...
	else
	{
		...
		if (nullptr == pShape)
		{
			std::string szMsg = fmt::format("Object NPC In : [Name({}), ResourceID({})]", szName, iIDResrc);
			MsgOutput(szMsg, 0xffff00ff);
			CLogWriter::Write(szMsg);
	...
	{
		std::string szMsg = fmt::format("No Resource NPC In : [Name({}), ResourceID({})]", szName, iIDResrc);
		CLogWriter::Write(szMsg);
	}
...
bool CGameProcMain::MsgRecv_NPCInAndRequest(Packet& pkt)
{
	...
	//TRACE("NPC region update : {}", iNPCCountReceived);
...
bool CGameProcMain::MsgRecv_NPCMove(Packet& pkt)
{
	...
	float fXPos = (pkt.read<uint16_t>()) / 10.0f;
	float fZPos = (pkt.read<uint16_t>()) / 10.0f;
	pkt.read<int16_t>(); // float fYPos = (pkt.read<int16_t>()) / 10.0f;
...
bool CGameProcMain::MsgRecv_Attack(Packet& pkt)
{
	...
	if (0x0 == iResult)               // 공격 실패
	{
		...
		if (pTarget->IsAlive() && pTarget->State() == PSA_SPELLMAGIC)
		{
		...
		{
			std::string szMsg = fmt::format_text_resource(IDS_MSG_FMT_TARGET_ATTACK_FAILED, pTarget->IDString());
...
bool CGameProcMain::MsgRecv_UserLookChange(Packet& pkt)
{
		...
		else
		{
			...
			else
			{
				if (PART_POS_HAIR_HELMET == ePartPos)
					...
					pUPC->PartSet(ePartPos, pLooks->szPartFNs[ePartPos], nullptr, nullptr);
			}
		}
	...
	{
		CLogWriter::Write("CGameProcMain::MsgRecv_UserLookChange() - Unknown Slot({}) Item({})", static_cast<int>(eSlot), dwItemID);
		return false;
	}
...
void CGameProcMain::MsgRecv_MyInfo_MSP(Packet& pkt)
{
	...
	bool bUseMP           = true;
	if (CLASS_REPRESENT_WARRIOR == eCR || CLASS_REPRESENT_ROGUE == eCR)
		bUseMP = false;
...
void CGameProcMain::MsgRecv_MyInfo_RealmPoint(Packet& pkt)
{
	...
	else if (opcode == LOYALTY_CHANGE_MANNER)
	{
		...
		CLogWriter::Write("Got manner update packet but didn't update UI since there is no manner feature. New Manner: {}.", iNewManner);
	}
	else
	{
...
{
	__WarpInfo WI;
	if (!m_pUIWarp->InfoGetCur(WI) || WI.szName.empty())
		return;
	...
	{
		std::string szMsg = fmt::format_text_resource(IDS_TELEPORT_TO_X_NEED_Y_COINS, WI.szName, WI.iGold);
		MsgOutput(szMsg, 0xFFFF3B3B);
		return;
	}
...
void CGameProcMain::InitZone(int iZone, const __Vector3& vPosPlayer)
{
	...
	if (iZonePrev != iZone)   // 다른 존일 경우에만 로딩..
	{
		...
		UpdateUI_PartyOrForceButtons();                       // 커맨드 줄에 있는 파티 버튼을 상황에 따라 업데이트 해준다.
...
void CGameProcMain::CommandMove(e_MoveDirection eMD, bool bStartOrEnd)
{
	...
	if (MD_FORWARD == eMD || MD_BACKWARD == eMD)
	{
		...
		{
			float fSpeed =
				s_pPlayer->MoveSpeedCalculationAndCheckCollision(); // 속도를 구하고 그 속도로 충돌 체크를 한다. 리턴값이 0 이면 충돌이다..
...
void CGameProcMain::StartAutoAttack(CPlayerBase* target)
{
	...
	std::string szMsg = fmt::format_text_resource(IDS_MSG_ATTACK_START, target->IDString());
...
bool CGameProcMain::OpenCmdEdit(const std::string& msg)
{
...
bool CGameProcMain::MsgRecv_ItemDroppedGetResult(Packet& pkt) // 땅에 떨어진 아이템 먹기 결과..
{
	...
	if (m_pUIDroppedItemDlg != nullptr)
		m_pUIDroppedItemDlg->GetItemByIDToInventory(bResult, iItemID, iGoldID, bPos, sItemCount, szString);
	return true;
}
...
void CGameProcMain::MsgRecv_ZoneChange(Packet& pkt)
{
	...
	switch (opcode)
	{
		case ZONE_CHANGE_TELEPORT:
		{
			...
			uint8_t byBuff[4];
			int iSendOffset = 0;
			CAPISocket::MP_AddByte(byBuff, iSendOffset, WIZ_ZONE_CHANGE);
			CAPISocket::MP_AddByte(byBuff, iSendOffset, (uint8_t) ZONE_CHANGE_LOADING);
			s_pSocket->Send(byBuff, iSendOffset);
		}
		break;
		...
		case ZONE_CHANGE_LOADED:
		{
			...
			int iSendOffset = 0;
			CAPISocket::MP_AddByte(byBuff, iSendOffset, WIZ_ZONE_CHANGE);
			CAPISocket::MP_AddByte(byBuff, iSendOffset, (uint8_t) ZONE_CHANGE_LOADED);
			s_pSocket->Send(byBuff, iSendOffset);
		}
		break;
...
void CGameProcMain::MsgRecv_UserState(Packet& pkt)
{
	...
	else if (N3_SP_STATE_CHANGE_ACTION == eSP)       // 크기 변함
	{
		if (1 == iState)
			...
			pBPC->AnimationAdd(ANI_GREETING1, true); // 인사
		else if (3 == iState)
			pBPC->AnimationAdd(ANI_GREETING2, true); // 인사
		else if (11 == iState)
			pBPC->AnimationAdd(ANI_WAR_CRY1, true);  // 도발
		else if (12 == iState)
			pBPC->AnimationAdd(ANI_WAR_CRY2, true);  // 도발
		else if (13 == iState)
			pBPC->AnimationAdd(ANI_WAR_CRY3, true);  // 도발
	}
	...
	switch (iSubCmd)
	{
		...
		case N3_SP_PARTY_OR_FORCE_REMOVE: // 0x04	// Send - s1(ID) | Recv - s1(ID) -
		{
			...
			if (iID == s_pPlayer->IDNumber())
			{
				...
				MsgOutput(szMsg, D3DCOLOR_ARGB(255, 255, 255, 255));              // 파티 해제 메시지
			...
			else
			{
				...
				if (pInfo)
				{
					...
					MsgOutput(pInfo->szID + szMsg, D3DCOLOR_ARGB(255, 255, 255, 255)); // 누가 파티에서 떠났다는 메시지..
...
void CGameProcMain::CommandSitDown(bool bLimitInterval, bool bSitDown, bool bImmediately)
{
	...
	if (bSitDown)
	{
		...
		if (m_pMagicSkillMng->IsCasting())                // 스킬 캐스팅 중이면 앉지 못한다..
			return;

		if (s_pPlayer->m_bAttackContinous)                // 계속 공격하는 중이면..
			CommandEnableAttackContinous(false, nullptr); // 계속 공격 취소..
	}
...
{
	int16_t sOtherID = 0, sItemCount = 0, sCount = 0, sDurability = 0;
	uint8_t bResult = 0, bItemPos = 0;
	int iItemID = 0, iCount = 0, iTotalGold = 0;
...
void CGameProcMain::ParseChattingCommand(const std::string& szCmd)
{
	...
	switch (eCmd)
	{
		...
		case CMD_PARTY:
		{
			...
			{
				std::string szMsg;
				if (MsgSend_PartyOrForceCreate(pTarget->IDString()))
					szMsg = fmt::format_text_resource(IDS_PARTY_INVITE);        // 파티 요청..
				else
					szMsg = fmt::format_text_resource(IDS_PARTY_INVITE_FAILED); // 파티 초대 실패
				MsgOutput(pTarget->IDString() + szMsg, 0xffffff00);
			}
		}
		break;

		case CMD_LEAVEPARTY:
			MsgSend_PartyOrForceLeave(); // 파티 요청..
			break;

		case CMD_RECRUITPARTY:
		{
			if (m_pUIPartyBBS != nullptr)
			{
...
void CGameProcMain::UpdateUI_PartyOrForceButtons()
{
	...
	int iMemberIndex = -1;
	PartyOrForceConditionGet(bIAmLeader, bIAmMemberOfParty, iMemberIndex, pTarget);
...
{
	if (nullptr == m_pUIStateBarAndMiniMap || !m_pUIStateBarAndMiniMap->IsVisible())
		return;
...
{
	if (nullptr == m_pUITargetBar || !m_pUITargetBar->IsVisible())
		return;
...
{
	uint8_t bResult = 0, bSubCom = 0;
	bSubCom = pkt.read<uint8_t>();
...
void CGameProcMain::MsgRecv_WareHouseOpen(Packet& pkt) // 보관함 오픈..
{
	...
	int iWareGold = 0, iItemID = 0, iItemDurability = 0, iItemCount = 0;
...
void CGameProcMain::MsgRecv_SkillPointInit(Packet& pkt) // Skill Point 초기화..
{
	...
	switch (bType)
	{
		...
		case 0x01: // 성공..
			...
			if (m_pSubProcPerTrade != nullptr && m_pSubProcPerTrade->m_pUIPerTradeDlg->IsVisible())
				m_pSubProcPerTrade->m_pUIPerTradeDlg->GoldUpdate();
...
void CGameProcMain::MsgSend_PerTradeBBSReq(const std::string& szName, int iDestID)
{
...
void CGameProcMain::ControlViewVDegree(int16_t sValue)
{
	if (s_pEng->ViewPoint() == VP_THIRD_PERSON)
	{
		float fPitch = 0;
		if (sValue > 0)
			fPitch = DegreesToRadians(45.0f) * 3;
		else
			fPitch = DegreesToRadians(-45.0f) * 3;
		if (fPitch)
			s_pEng->CameraPitchAdd(fPitch);
...
bool CGameProcMain::OnMouseLbtnDown(POINT ptCur, POINT /*ptPrev*/)
{
		...
		&& !s_pLocalInput->IsKeyDown(KM_MOVE_FOWARD)) // 삼인칭 시점이면.. UI 를 건들지 않았으면..
	{
		...
		if (fNowTime - m_fLBClickTime > 0.1f && s_pPlayer->m_bTargetOrPosMove)
		{
			...
			CommandSitDown(false, false); // 일단 일으켜 세운다..
			s_pPlayer->RotateTo(fYaw, true);
			CommandMove(MD_FORWARD, false);
			...
			if (pShape && pShape == s_pPlayer->m_pObjectTarget && pShape->m_iEventID) // Event 가 있으면..
			{
				...
				else
				{
					if (OBJECT_TYPE_BINDPOINT == pShape->m_iEventType)
					{
						...
						MessageBoxPost(szMsg, "", MB_YESNO, BEHAVIOR_REQUEST_BINDPOINT); // 바인팅 포인트 설정 메시지 박스
					}
					else if (OBJECT_TYPE_WARP_POINT == pShape->m_iEventType)
					{
						MsgSend_ObjectEvent(pShape->m_iEventID, pShape->m_iNPC_ID); // 오브젝트 이벤트 보내기..
					}
				}
			}
		}
...
bool CGameProcMain::OnMouseRbtnDown(POINT ptCur, POINT ptPrev)
{
	...
	if (fRotY != 0.0f && s_pPlayer->IsAlive())
	{
		if (VP_THIRD_PERSON == s_pEng->ViewPoint())
			s_pEng->CameraYawAdd(fRotY);
		// 기절해 있지 않을때만..
